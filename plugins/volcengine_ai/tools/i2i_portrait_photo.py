...
class I2IPortraitPhotoTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        if not image_input:
            self.create_text_message("image_file is required")

        form = {
            "req_key": "i2i_portrait_photo",
            "image_input": image_input,
            "prompt": tool_parameters.get("prompt", "演唱会现场的合照，闪光灯拍摄"),
            "width": tool_parameters.get("width", 1328),
            "height": tool_parameters.get("height", 1328),
            "gpen": tool_parameters.get("gpen", 0.4),
            "skin": tool_parameters.get("skin", 0.3),
            "skin_unifi": tool_parameters.get("skin_unifi", 0),
            "gen_mode": tool_parameters.get("gen_mode", ""),
